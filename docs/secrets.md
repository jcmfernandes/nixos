...
| `&<host>`  | The host's `/etc/ssh/ssh_host_ed25519_key` | Decrypting at NixOS activation |
...
The whole point of sops-nix is that nothing sensitive lands in git as
plaintext. `gitleaks` (in the devenv shell) verifies that across the full
history:
...
`.gitleaks.toml` extends the default ruleset with an allowlist for the two
things that *look* like leaks but aren't:

- `secrets/*.yaml` — sops ciphertext, meant to be in git.
- `age-identities` — a YubiKey-backed age identity. The
  `AGE-PLUGIN-YUBIKEY-...` stub points at the token (serial + slot); the
  private key never leaves the hardware, so the string is useless without
  the physical YubiKey + PIN + touch.

A clean run is `no leaks found`. Anything else is a real plaintext secret
that needs scrubbing from history (and rotating).
